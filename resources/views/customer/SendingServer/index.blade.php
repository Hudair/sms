    ...
    <section id="data-list-view" class="data-list-view-header">
        <div class="action-btns d-none">
            <div class="btn-dropdown mr-1 mb-1 add-new-div">
                ...
                @can('create_sending_servers')
                    <div class="btn-group dropdown actions-dropodown">
                        <a href="{{route('customer.sending-servers.select')}}" class="btn btn-white px-1 py-1 waves-effect waves-light text-success text-bold-500">{{__('locale.buttons.new_server')}} <i class="feather icon-plus-circle"></i></a>
                    </div>
                @endcan
            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            let dataListView = $('.data-list-view').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('customer.sending-servers.search') }}",
                    "dataType": "json",
                    "type": "POST",
            ...
            Table.delegate(".get_status", "click", function () {
                ...
                $.ajax({
                    url: "{{ url('/sending-servers')}}" + '/' + sending_server_id + '/active',
                    type: "POST",
                    data: {
                        _token: "{{csrf_token()}}"
                    },
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        if (sending_server_ids.length > 1) {
                            ...
                            $.ajax({
                                url: "{{ route('customer.sending-servers.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'enable',
