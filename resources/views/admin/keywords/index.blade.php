...
@section('content')
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            @can('view keywords')
                <div class="btn-group">
                    ...
                    <div class="dropdown-menu" aria-labelledby="bulk_actions">
                        ...
                        <a class="dropdown-item bulk-available" href="#"><i data-feather="check"></i> {{ __('locale.labels.available') }}</a>
            ...
            @can('create keywords')
                <div class="btn-group">
                    <a href="{{route('admin.keywords.create')}}" class="btn btn-success waves-light waves-effect fw-bold mx-1"> {{__('locale.buttons.create')}} <i data-feather="plus-circle"></i></a>
                </div>
            @endcan

            @can('view keywords')
                <div class="btn-group">
                    <a href="{{route('admin.keywords.export')}}" class="btn btn-info waves-light waves-effect fw-bold"> {{__('locale.buttons.export')}} <i data-feather="file-text"></i></a>
                </div>
            @endcan
        ...
        $(document).ready(function () {
                        ...
                        render: function (data, type, full) {
                            ...
                            if (full['reply_mms'] === true) {
                                $actions = '<span class="action-remove-mms text-warning cursor-pointer" data-bs-toggle="tooltip" data-placement="top" title="' + full['remove_mms'] + '" data-id=' + full['uid'] + '>' +
                                    feather.icons['file-minus'].toSvg({class: 'font-medium-4'}) +
                                    '</span>';
                            }
                            return (
                                $actions +

                                '<a href="' + full['show'] + '" class="text-primary mx-1" data-bs-toggle="tooltip" data-placement="top" title=' + full['show_label'] + '>' +
                                feather.icons['edit'].toSvg({class: 'font-medium-4'}) +
                                '</a>' +
                                '<span class="action-delete text-danger cursor-pointer" data-bs-toggle="tooltip" data-placement="top" title=' + full['delete'] + ' data-id=' + full['uid'] + '>' +
            ...
            Table.delegate(".action-remove-mms", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/keywords')}}" + '/' + id + '/remove-mms',
                            type: "POST",
                            data: {
                                _token: "{{csrf_token()}}"
                            },
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/keywords')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
            ...
            $(".bulk-available").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.keywords.available_keywords')}}",
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.value) {
                        ...
                        if (keyword_ids.length > 0) {
                            ...
                            $.ajax({
                                url: "{{ route('admin.keywords.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'available',
            ...
            $(".bulk-delete").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.keywords.delete_keywords')}}",
                    icon: 'warning',
                    showCancelButton: true,
