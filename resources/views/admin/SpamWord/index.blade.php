...
@section('title', __('locale.menu.Spam Word'))
...
@section('content')
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            @can('create spam_word')
                <div class="btn-group">
                    <a href="{{route('admin.spam-word.create')}}" class="btn btn-success waves-light waves-effect fw-bold mx-1"> {{__('locale.buttons.add_new')}} <i data-feather="plus-circle"></i></a>
                </div>
            @endcan

            @can('view spam_word')
                <div class="btn-group">
                    <a href="{{route('admin.spam-word.export')}}" class="btn btn-info waves-light waves-effect fw-bold"> {{__('locale.buttons.export')}} <i data-feather="file-text"></i></a>
                </div>
            @endcan
                ...
                <div class="card">
                    <table class="table datatables-basic">
                        ...
                        <tr>
                            ...
                            <th>{{__('locale.menu.Spam Word')}} </th>
    ...
    <script>
        $(document).ready(function () {
            ...
            let dataListView = $('.datatables-basic').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('admin.spam-word.search') }}",
                    "dataType": "json",
                    "type": "POST",
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/spam-word')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
            ...
            $(".bulk-delete").on('click', function (e) {
                ...
                Swal.fire({
                    ...
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.spam_word.delete_words')}}",
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.value) {
                        ...
                        $.each(rows_selected, function (index, rowId) {
                            spam_word_ids.push(rowId)
                        });

                        if (spam_word_ids.length > 0) {

                            $.ajax({
                                url: "{{ route('admin.spam-word.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'destroy',
