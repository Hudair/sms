    ...
    <section id="data-list-view" class="data-list-view-header">
        <div class="action-btns d-none">
            <div class="btn-dropdown mr-1 mb-1 add-new-div">
                ...
                @can('create blacklist')
                    <div class="btn-group dropdown actions-dropodown">
                        <a href="{{route('admin.blacklists.create')}}" class="btn btn-white px-1 py-1 waves-effect waves-light text-success text-bold-500"> {{__('locale.buttons.add_new')}} <i class="feather icon-plus-circle"></i></a>
                    </div>
                @endcan

                @can('view blacklist')
                    <div class="btn-group dropdown actions-dropodown">
                        <a href="{{route('admin.blacklists.export')}}" class="btn btn-white px-1 py-1 waves-effect waves-light text-info text-bold-500"> {{__('locale.buttons.export')}} <i class="feather icon-file-text"></i></a>
                    </div>
                @endcan
        ...
        <div class="table-responsive">
            <table class="table data-list-view">
                ...
                <tr>
                    ...
                    <th>{{__('locale.labels.listed_by')}} </th>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            let dataListView = $('.data-list-view').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('admin.blacklists.search') }}",
                    "dataType": "json",
                    "type": "POST",
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/blacklists')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        if (sender_ids.length > 1) {
                            ...
                            $.ajax({
                                url: "{{ route('admin.blacklists.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'destroy',
