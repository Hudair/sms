...
@section('title', __('locale.menu.Plan'))
        ...
        <div class="action-btns d-none">
            <div class="btn-dropdown mr-1 mb-1 add-new-div">
                ...
                @can('create sender_id')
                    <div class="btn-group dropdown actions-dropodown">
                        <a href="{{route('admin.senderid.create_plan')}}" class="btn btn-white px-1 py-1 waves-effect waves-light text-success text-bold-500"> {{__('locale.labels.create_plan')}} <i class="feather icon-plus-circle"></i></a>
                    </div>
                @endcan
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            let dataListView = $('.data-list-view').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('admin.senderid.search_plan') }}",
                    "dataType": "json",
                    "type": "POST",
                ...
                "columns": [
                    ...
                    {"data": "price"},
                    {"data": "renew", orderable: false, searchable: false},
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/senderid/delete-plan')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _token: "{{csrf_token()}}"
                            },
            ...
            $(".bulk-delete").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.sender_id.delete_senderid_plans')}}",
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    if (result.value) {
                        ...
                        dataListView.rows('.selected').every(function (rowIdx) {
                            plan_ids.push(dataListView.row(rowIdx).data().uid)
                        })

                        if (plan_ids.length > 1) {

                            $.ajax({
                                url: "{{ route('admin.senderid.delete_batch_plan') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    ids: plan_ids
